...
- **운영 측정 기준:** #568 적용 후 pre-market 실행 로그의 최근 7일 최댓값을 관측하고 `최댓값 / 3600초`가 0.8 이상이면 경고, 1.0 이상이면 현재 TTL을 완료 근거로 사용하지 않는다. 1.0 이상이거나 재시도 정책 변경으로 초과가 예상되면 배치별 TTL 또는 lease-renewal을 별도 ADR로 검토한다.
...
- [x] **6. 실제 Redis 기반 네 서비스 동시성 통합 테스트**
  ...
  동시성 테스트는 기존 `@Transactional` 통합 테스트에 얹지 않고 별도 비트랜잭션 테스트로 둔다. `@BeforeEach`·`@AfterEach`에서 Redis 키와 테스트 DB를 정리하고, `CountDownLatch`로 시작점을 맞추며, 스레드별 예외·호출 횟수·저장 호출을 모두 확인한다. 테스트 성공을 #568 이후의 실제 다중 인스턴스 배포 검증으로 표현하지 않는다.

- [x] **7. 문서·완료 정합성 및 검증 게이트**
